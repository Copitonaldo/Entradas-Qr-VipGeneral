...
async function cargarRespuestas() {
  ...
  // Cargar respuestas y unirlas con el tipo de entrada de la referencia usada
  ...
  if (respuestasError) {
    ...
    // Mapear respuestas para incluir tipo_entrada
    todasLasRespuestas = await Promise.all(respuestasData.map(async (r) => {
        let tipoEntrada = 'N/A'; // Valor por defecto si no hay referencia o no se encuentra
        if (r.referencia_usada) {
            // Buscar el tipo de entrada correspondiente a la referencia usada
            const { data: refData, error: refError } = await supabase
                .from('referencias_usos')
                .select('tipo_entrada')
                .eq('formulario_id', currentFormDbId)
                .eq('codigo_referencia', r.referencia_usada)
                .single();
            if (!refError && refData) {
                tipoEntrada = refData.tipo_entrada;
            }
        }
        return { 
            id_db: r.id, 
            ...
            tipo_entrada: tipoEntrada // <-- Nuevo campo
        };
    }));
  }
